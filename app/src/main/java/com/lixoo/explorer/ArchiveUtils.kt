...
object ArchiveUtils {
    ...
    private val archiveCache = mutableMapOf<String, Map<String, List<ArchiveEntryInfo>>>()
    ...
    fun listContents(archiveFile: File, internalPath: String): List<ArchiveEntryInfo> {
        val archivePath = archiveFile.absolutePath
        val cached = archiveCache[archivePath]
        if (cached != null) {
            return cached[internalPath] ?: emptyList()
        }

        val fullList = mutableListOf<ArchiveEntryInfo>()
        if (DiskImageUtils.isDiskImage(archiveFile)) {
            fullList.addAll(DiskImageUtils.listDiskContents(archiveFile))
        } else {
            val name = archiveFile.name.lowercase()
            val result = when {
                name.endsWith(".7z") -> list7zContents(archiveFile)
                name.endsWith(".rar") -> listStandardArchiveContents(archiveFile, "rar")
                ...
                name.endsWith(".gz") || name.endsWith(".bz2") || name.endsWith(".xz") || name.endsWith(".lz4") -> {
                    listOf(ArchiveEntryInfo(archiveFile.nameWithoutExtension, false, archiveFile.length(), archiveFile.lastModified(), archiveFile.nameWithoutExtension))
                }
                else -> emptyList()
            }
        ...
        if (fullList.isNotEmpty()) {
            val grouped = groupEntries(fullList)
            archiveCache[archivePath] = grouped
            return grouped[internalPath] ?: emptyList()
        }
        return emptyList()
    }

    private fun groupEntries(fullList: List<ArchiveEntryInfo>): Map<String, List<ArchiveEntryInfo>> {
        val map = mutableMapOf<String, MutableList<ArchiveEntryInfo>>()
        val seen = mutableSetOf<String>()

        fullList.forEach { entry ->
            val fullName = (if (entry.fullName.isEmpty()) entry.name else entry.fullName).removeSuffix("/")
            ...
            for (i in parts.indices) {
                ...
                val key = "$parent|$name"
                if (seen.add(key)) {
                    val info = if (isLast) entry.copy(name = name, fullName = fullName)
                               else ArchiveEntryInfo(name, true, 0, 0, currentPath)
                    map.getOrPut(parent) { mutableListOf() }.add(info)
                }
            }
        }
        return map
    ...
    private fun listStandardArchiveContents(file: File, format: String): List<ArchiveEntryInfo> {
        ...
        val inputStream = try {
            when (format) {
                ...
                else -> ArchiveStreamFactory().createArchiveInputStream(format, bis)
            }
        } catch (e: Exception) {
            bis.close()
            return emptyList()
    ...
    fun extract(archiveFile: File, outputDir: File, entryName: String? = null) {
        ...
        when {
            ...
            name.endsWith(".rar") -> extractStandard(archiveFile, outputDir, "rar", entryName)
            ...
            else -> extractGeneric(archiveFile, outputDir, entryName)
        }
    }
    ...
    private fun extractStandard(file: File, outputDir: File, format: String, targetEntry: String?) {
        val fis = FileInputStream(file)
        val bis = BufferedInputStream(fis)
        val inputStream: ArchiveInputStream<*> = try {
            when (format) {
                "zip" -> ZipArchiveInputStream(bis)
                "tar" -> TarArchiveInputStream(bis)
                else -> ArchiveStreamFactory().createArchiveInputStream(format, bis)
