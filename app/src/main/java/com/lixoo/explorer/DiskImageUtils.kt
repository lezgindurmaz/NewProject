...
object DiskImageUtils {
    ...
    class Qcow2Disk(private val raf: RandomAccessFile) : VirtualDisk {
        ...
        init {
            ...
            for (i in 0 until Math.min(l1Size, l1Buf.size / 8)) {
                l1Table[i] = getLongBE(l1Buf, i * 8) and 0x00fffffffffffe00L
            }
            // If L1 is larger than 1MB, read the rest individually or in chunks
            if (l1Size * 8 > l1Buf.size) {
                raf.seek(l1TableOffset + l1Buf.size)
                for (i in (l1Buf.size / 8) until l1Size) {
                    l1Table[i] = raf.readLong() and 0x00fffffffffffe00L
                }
            }
        }
            ...
            while (totalRead < len) {
                ...
                if (cachedClusterPos != clusterBase) {
                    ...
                    if (phys <= 0L) {
                        ...
                        raf.seek(phys)
                        val r = raf.read(clusterBuffer)
                        if (r < clusterSize && r > 0) {
                            for (i in r until clusterSize) clusterBuffer[i] = 0
                        }
                    }
        ...
        private fun getPhysicalOffset(vPos: Long): Long {
            ...
            val l2Table = if (l2Cache.containsKey(l2TablePos)) l2Cache[l2TablePos]!! else {
                if (l2Cache.size > 128) l2Cache.clear()
            ...
            if ((entry and (1L shl 62)) != 0L) return -2L // Compressed

            return entry and 0x00fffffffffffe00L
        }
    ...
    fun listDiskContents(file: File): List<ArchiveUtils.ArchiveEntryInfo> {
        ...
        try {
            val fatOffset = findFatOffset(disk)
            if (fatOffset != -1L) {
                return parseFatContents(disk, fatOffset)
            }
    ...
    private fun findFatOffset(disk: VirtualDisk): Long {
        val b = ByteArray(512)
        disk.seek(0)
    ...
    private fun isFatBPB(b: ByteArray): Boolean {
        val sig = (b[510].toInt() and 0xFF == 0x55) && (b[511].toInt() and 0xFF == 0xAA)
        if (!sig) return false
        val jmp = b[0].toInt() and 0xFF
        return jmp == 0xEB || jmp == 0xE9
    }

    private fun parseFatContents(disk: VirtualDisk, baseOffset: Long): List<ArchiveUtils.ArchiveEntryInfo> {
        val items = mutableListOf<ArchiveUtils.ArchiveEntryInfo>()
        try {
            val bpb = ByteArray(512)
            disk.seek(baseOffset)
            disk.read(bpb, 0, 512)

            val bytesPerSector = getShortLE(bpb, 11).toInt()
            if (bytesPerSector == 0) return emptyList()
            ...
            val sectorsPerFat = if (sectorsPerFat16 != 0) sectorsPerFat16.toLong() else getIntLE(bpb, 36).toLong()

            val rootDirStart = baseOffset + (reservedSectors + (numFats * sectorsPerFat)) * bytesPerSector
            ...
            val fatStart = baseOffset + reservedSectors.toLong() * bytesPerSector

            val totalSectors = if (getShortLE(bpb, 19).toInt() != 0) getShortLE(bpb, 19).toInt().toLong() and 0xFFFF else getIntLE(bpb, 32).toLong() and 0xFFFFFFFFL
    ...
    private fun extractFromFat(file: File, outputDir: File, entryName: String?) {
        ...
        try {
            ...
            val rootDirStart = fatOffset + (reservedSectors + (numFats * sectorsPerFat)) * bytesPerSector
            val rootDirSectors = ((rootEntries * 32) + (bytesPerSector - 1)) / bytesPerSector
            val dataStart = rootDirStart + (rootDirSectors * bytesPerSector)
            val fatStart = fatOffset + reservedSectors.toLong() * bytesPerSector
